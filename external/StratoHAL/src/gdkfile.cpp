...
/*
 * GDK Implementation of the File Access
 *
 * This implementation supports two read sources:
 *   - Packaged assets (MSIXVC) via XPackageMount() (handled by open_package())
 *   - Cloud-synced saves via XGameSaveFilesGetFolderWithUiAsync() + Win32 file I/O
 *
 * Notes:
 *   - XGameSaveFiles provides a *folder path* where the title can use standard
 *     Win32 file I/O (CreateFile/ReadFile/WriteFile).
 *   - Files must be stored under a *subdirectory* (container). Files directly
 *     under the returned root folder are ignored for cloud storage.
 */
