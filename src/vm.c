...
static bool load_startup_file(void)
{
	...
	if (!rt_register_source(rt, STARTUP_FILE, buf)) {
		log_error(_("%s:%d: error: %s"),
			  rt_get_error_file(rt),
			  rt_get_error_line(rt),
			  rt_get_error_message(rt));
...
static bool API_loadTexture(struct rt_env *rt)
{
	...
	if (!get_string_param(rt, "file", &file)) {
		rt_error(rt, _("file parameter is not set."));
		return false;
	}

	if (!load_texture(file, &tex_id, &tex_width, &tex_height)) {
		rt_error(rt, _("Failed to load a texture."));
		return false;
	}
...
#define RTFUNC(name) {API_##name, #name, "api_" # name}
	...
	for (i = 0; i < tbl_size; i++) {
		...
		/* Register a cfunc. */
		if (!rt_register_cfunc(rt, funcs[i].name, 1, params, funcs[i].func, NULL))
			return false;
